...
from .._date_ranges import rango_mundial, rango_mundial_sub20
from ...core.utils import safe_get, normalize_team


def _fifa_standings(self) -> Optional[dict]:
...
class MundialScraper(BracketMixin, BaseCompetitionScraper):
    slug = "mundial"
    league_code = "fifa.world"
    name = "Copa del Mundo FIFA"
    def _date_ranges(self): return rango_mundial()
    fetch_standings = _fifa_standings


class MundialClubesScraper(BracketMixin, BaseCompetitionScraper):
    slug = "mundial-clubes"
    league_code = "fifa.cwc"
    name = "Mundial de Clubes FIFA"
    def _date_ranges(self): return rango_mundial()
    fetch_standings = _fifa_standings


class MundialSub20Scraper(BracketMixin, BaseCompetitionScraper):
    slug = "mundial-sub20"
    league_code = "fifa.world.u20"
    name = "Mundial Sub-20 FIFA"
    def _date_ranges(self): return rango_mundial_sub20()
    fetch_standings = _fifa_standings
